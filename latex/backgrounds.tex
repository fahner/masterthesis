...
Being aware of these problems due to experimental setup it is clear that one cannot use a well reconstructed \Lb mass peak to separate signal from background.
...
\end{figure}
To determine a final value for each identification efficiency, the histograms are weighted with respect to the kinematic distribution of the \LbToDpmunuX data.
The result is:
...
These backgrounds are not easy to eliminate, since the other 2 pions are not reconstructed and thus not peaking in \Dz\proton\mun mass.
...
\section{Prompt \Dz}
With prompt \Dz one denotes \Dz coming directly from the primary \proton\proton interaction, thus being not a product of other decaying particles.
...
However, Figure \ref{fig:plot_D0_IP} does not show any discrepancies between data and different wrong sign combinations, which would indicate the influence of prompt \Dz backgrounds.
Compared to other background sources this kind of background seems to be negligible, since there are furthermore additional selection requirements applied in the reconstruction to combine the \Dz with the other particles to form a \Lb candidate.
\begin{figure}[hptb]
	\centering
...
\begin{itemize}
    \item $\Lambda_b^0 \rightarrow \Dm \Lc$ \\
          This decay has not been measured yet, but it should have a branching ratio of around $10^{-2}$, similar to $B \rightarrow D D_s$.
